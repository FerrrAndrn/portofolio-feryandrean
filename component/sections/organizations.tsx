...
interface OrganizationItem {
  ...
  logos: string[];
  cabinet?: string;
  bullets?: string[]; // ✅ dibuat optional supaya openItem.bullets?.length aman
            ...
            <div className="flex items-center justify-center gap-6 mb-3">
              {org.logos.map((logo, idx) => (
                <img
                  ...
                  src={logo}
                  alt={`logo ${idx + 1}`}
                  className={`${idx === 0 ? "w-20 h-20" : "w-24 h-24"} object-contain drop-shadow`}
                />
              ))}
            </div>

            {org.cabinet && <p className="text-sm font-bold text-gray-800 mb-3">{org.cabinet}</p>}
              ...
              >
                ...
                >
                  ...
                  <div className="text-center mb-6">
                    <p className="text-base font-bold text-[#001f3f]">{openItem.org}</p>
                    <h3 className="text-sm font-semibold text-gray-700">{openItem.role}</h3>
                    <p className="text-sm font-semibold text-gray-700">{openItem.period}</p>
                    ...
                    <div className="flex items-center justify-center gap-6 mb-3">
                      {openItem.logos.map((logo, idx) => (
                        <img
                          ...
                          src={logo}
                          alt={`logo ${idx + 1}`}
                          className={`${idx === 0 ? "w-24 h-24" : "w-36 h-36"} object-contain drop-shadow`}
                        />
                      ))}
                    </div>

                    {openItem.cabinet && (
                      <p className="text-sm font-bold text-gray-800">{openItem.cabinet}</p>
                    )}
                  </div>

                  <div className="border-t border-gray-300 pt-6 mt-6">
                    <h4 className="font-bold text-[#001f3f] mb-4 text-center uppercase">{t("jobdesc")}</h4>
                    <ul className="list-disc pl-5 space-y-2 text-gray-700 text-left">
                      {openItem.bullets?.length ? (
                        ...
                        <li className="italic text-gray-500">{t("noDetailsAvailable") || "Detail belum tersedia"}</li>
                      )}
                    </ul>
                  </div>

                 {openItem.documentation && openItem.documentation.length > 0 && (
                    <div className="border-t border-gray-300 pt-6 mt-6">
                      <h4 className="font-bold text-[#001f3f] mb-4 text-center uppercase">{t("documentation")}</h4>
                      <Swiper
                        spaceBetween={20}
                        slidesPerView={3}
                        navigation
                  ...
                  {openItem.certificate && openItem.certificate.length > 0 && (
                    <div className="border-t border-gray-300 pt-6 mt-6 text-center">
                      <h4 className="font-bold text-[#001f3f] mb-4 text-center uppercase">{t("certificate")}</h4>
